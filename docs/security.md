...
| Area | Control | Where |
|------|---------|-------|
| Transport / headers | HSTS, `X-Content-Type-Options`, `X-Frame-Options: DENY`, `Referrer-Policy`, `Permissions-Policy`. | `infra/cloudflare/worker.js` (live), `infra/aws/main.tf` CloudFront response-headers policy (pending account verification), [ADR-0018](./adr/0018-portable-static-export.md) |
| XSS | Static CSP header: `default-src 'self'`, no external script/connect/frame origins, `object-src 'none'`, `frame-ancestors 'none'`. `'unsafe-inline'` for scripts is required by static generation and accepted — the site holds no sessions or user data. | `infra/cloudflare/worker.js` (live), `infra/aws/main.tf` CloudFront response-headers policy (pending account verification), [ADR-0018](./adr/0018-portable-static-export.md) |
...
5. Verify the deployed response headers before moving traffic between the
   Cloudflare Worker and CloudFront; both platforms must enforce the same
   policy.
